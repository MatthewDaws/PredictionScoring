...
We are interested in spatial ``predictive policing'' algorithms, \cite{rand}
[\footnote{Add some more general references}]
and in particular,
...
available at \texttt{https://github.com/QuantCrimAtLeeds/PredictCode}.
[\footnote{When finished, we should \emph{release} a fixed version, and also
put the notebooks etc. in a different github repo.}]
...
model; the ``reality'' we wish to compare to forms a sparse, stochastic point pattern.
...
[\footnote{Write a little summary box here, fixing notation.}]
...
(leading to the ``predictive accuracy index'', see \cite{ctu}).  As we shall only consider
a fixed study area, we shall continue to the coverage level and hit rate directly.
...
The authors then work directly with values $(a_i)$.  For example,
\begin{itemize}
...
\item The ``resolution'' of the forecast.  Suppose in Leeds that is actually rains around
...
To adapt this framework to our idea carries seems difficult.  Even assuming independence
...
As a thought experiment, we perform some computational experiments.  We
fix a real probability $p_{\text{r}}$ and $n>0$.  For each trial, we simulate
$N$ as a Poisson random variable with mean $n$, and then simulate $n_H$ (with
...
with value $p$.  We hence have three variables, $n, p, p_{\text{r}}$, and we
wish to see how the methods capture $p$ being close to $p_{\text{r}}$ (or not)
and what effect increasing $n$ (which should increase our confidence in assessing
a prediction) changes the result.
...
for $n$ not too small, the expected hit rate is $p$.  Thus here the hit rate
...
here increases the value of the score, but if normalised, has the effect of both
decreasing the variance, and of increasing the score when $p$ and $p_{\text{real}}$
differ.  We will see shortly that the KL divergence score for the Dirichlet prior
...
\item Complete spatial randomness, that is, a homogeneous Poisson process.  This is equivalent
  to setting $p_k = 1/K$ for all $k$.  When making a prediction, we add each $p_k$ a uniformly
...
In the current experiment, notice that it is impossible to ``predict'' better than
the generating model from which the data is actually drawn.  We have two graphical
...
\item The empirical ``cumulative density'' from the trials.  For each trial, we look at
  ...
  data from Model~1, around 20\% of the time, both predictions give the same
  score.  That the density appears symmetric suggests (correctly, in this case)
  that this assessment method does not discrimate well between the predictions.
\end{itemize}
...
\begin{figure*}
  ...
  \caption{Hit rate for 20\% coverage.}
   \label{fig:5}
\end{figure*}
...
If we select a fixed bandwidth (remember that this applied to the kernel estimation
...
scatter plots show dispersed kernel estimations, with the points forming a rough
...
the Brier score decreases in absolute terms, which the Skill score increases and becomes
...
distributions, and for the predictive distributions.  We will set $t = 4$, as this is about
...
As data, we shall use the publicly available dataset of crime event from Chicago.
We look will look at Burglary events in the North Side, for the year 2016.  Figure~\ref{fig:c1}
...
For each day of September to December, inclusive,
we make a prediction for that day, using data from January 2016 up to, but \emph{not} including,
...
estimation method, with a time component) and then sample to a grid.  We shall see that
...
levels, but the missing plots are all very similar.
...
somewhat confirms this, though we the jump in quality of the naive prediction at aggregation
...
At higher aggregation levels the naive method again seems to loose ground.  However, notice
that the $x$ axis scale becomes rather narrow (relative to the associated scatter plots).
Hence the ``real'' gradient of the CDF plot is becomes large, indicating a great deal of
...
The CDF plots do not change terrible much in how they discriminate between the two
...
(which could actually be a single algorithm, or many algorithms).  Let us model
...
the average hit rate, which is $\frac{1}{I} \sum_i x^{(l)}_i / n_i$.
...
\bibitem{hz} A. Hagen-êZanker, ``An improved Fuzzy Kappa statistic that accounts
...
\bibitem{jbmbp} S.\,D. Johnson, D.\,J. Birks, L. McLaughlin, K.\,J. Bowers, K. Pease,
	...
	Available at \texttt{http://library.college.police.uk/docs/hordsolr/rdsolr1907.pdf}
